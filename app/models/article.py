...
class Article(Base):
    ...
    # True when a scrape was attempted and failed (or was rejected as a
    # mis-scraped author bio - see app/review/scraping.py) AND the RSS
    # teaser (body_text) is also empty - i.e. there is genuinely no text
    # for a regular admin to review, blinded or not. app/review/
    # assignment.py never hands such an article to a regular admin's
    # queue (assigned_admin_id stays NULL); instead it shows up in the
    # super-admin-only "Manual Review" list with the raw source URL
    # (blinding doesn't apply there - a human has to actually visit the
